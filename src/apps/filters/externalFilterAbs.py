# # This is an abstract filter class for filers that relies on some parsing
# # from other services
# import pandas as pd
# import re
# import time
...
#         self.cleanColumns()
#         print(
#             f"Getting text from output column {self.relevantFields}. Please make sure they are correct")
...
#         start = time.time()
#         self.df[self.resultColumn] = self.df.progress_apply(
#             lambda row: self._filterTerms(row, terms, failedReason, successReason), axis=1)
#         stop = time.time()
#         print(
# f'Filtering {self.filterType} took {formatTime(start, stop)} seconds')
